...
TMatrixD CorrectCLAS_V(const TMatrixD &__VTrack,
   ...
   static double pPars[3][6][15][3],lamPars[3][6][15],phiPars[3][6][15];
   if(first_call) {
   ...
   if(__xid == "g12"){ 
     //res_scale = 2.1791;
     //p_scale = 1.5714;
     // res_scale = 2.2;  // Diane kaon parameter
     // p_scale = 1.94;
      res_scale = 2.83;
      p_scale = 1.94; 
      r_scale = 1.3;
   ...
   for(int i = 0; i < numParts; i++){      
    ...
    if (__p_names[i] == "gamma" && __xid == "g10a"){	
     ...
     if(__multi==false &&  __p_names[i]!="gamma" && __p_names[i]!="n" ) {                                     
       if( __p_names[i]=="pi+" || __p_names[i]=="pi-" || __p_names[i]=="k+" || __p_names[i] == "k-") part_index = 1; // use pion parameters
	 ...
	 if(p_mag > 0.4) sigma_eloss *= 0.8;
       ...
       if(part_index == 0 || part_index == 1) V(3*i+1,3*i+1) *= pow(p_res_scale,2);
	 ...
	 V(3*i+1,3*i+1) += pow(sigma_eloss/.765,2)
	   *(1.-.765*.765/2.)/(1.-.765*.765);
       ...
       if(part_index == 0 || part_index == 1) p_offset = 1.0*(pPars[part_index][sector-1][bin][2]);
     ...
     if(__multi==true &&  __p_names[i]!="gamma" &&  __p_names[i]!="n" &&  __p_names[i]!="e+" && __p_names[i]!="e-"){
     ...
     if(__multi==true &&  (__p_names[i]=="e+" || __p_names[i]=="e-")){
       Float_t res_scaleLam,res_scalePhi;	 
       p_res_scale = 0.02*r_scale;
       res_scaleLam = res_scale*0.1*r_scale;	 
       res_scalePhi = res_scale*0.6*r_scale;  
       sthetsq = (__p4[i].Px()*__p4[i].Px() + __p4[i].Py()*__p4[i].Py())/(p_mag*p_mag);
       ...
       // angular multiple scattering
       // add in multiple scattering errors 
       V(3*i+2,3*i+2) +=ms*0.011;
       V(3*i+3,3*i+3) +=0.00001*ms/sthetsq;
       
       // scale off diagnol elements by resolution scale factors
       V(3*i+1,3*i+2) *= 1;//res_scaleLam*p_res_scale; 
       V(3*i+2,3*i+1) = V(3*i+1,3*i+2);
       V(3*i+1,3*i+3) *= 1;//res_scalePhi*p_res_scale; 
       V(3*i+3,3*i+1) = V(3*i+1,3*i+3);
       V(3*i+2,3*i+3) *= 1;//res_scalePhi*res_scaleLam;
       V(3*i+3,3*i+2) = V(3*i+2,3*i+3);		
     }     
...
bool ReadInResParams(double __pPars[3][6][15][3],double __lamPars[3][6][15],
  ...
  String fileName_base = "/w/work/clas/clasg12/shankar/kpppim/kpppim/script/test/kinefit_badui/missing_kaon/KinFit/parms/";
  String fileName;
  String p_names[2];
  p_names[0] = "p";
  p_names[1] = "pi";
  ...
  for(int part = 0; part < 2; part++){
    ...
    while(*inFile >> sector){
      ...
      *inFile >> bin >> par[0] >> par[1] >> par[2];
      //cout<<sector<<"	"<<bin<<"	"<< par[0]<<"	" << par[1] << "	" << par[2] <<endl;
